...
def bonjour_monde
	...
	people = journalists.sample(5)
	people.each do |j|
	@client.update("#{j} merci pour votre rôle central dans le fonctionnement de notre démocratie ! #bonjour_monde @the_hacking_pro")
	end
end


def like_bonjour
	@client.search("#bonjour_monde -rt", result_type: "recent").take(1).collect do |hashtag|
		@client.favorite(hashtag)
	end
end	
 

def follow_bonjour
		@client.search("#bonjour_monde -rt", result_type: "recent").take(25).collect do |hashtag|
		@client.follow(hashtag.user)
	end
end
=end
...
def bonjour_moussaillons
	moussaillons =["@FralpsT", "@bakin_pancakes", "@Truccc5", "@Jahlawa"]

	mousse = moussaillons.sample(2)
	mousse.each do |m|
	@client.update("J'en ai plein le *** #{m}")
	end
end
